...
return array(
    ...
    //获取所有公告列表
    'getListByAnnouncementList'=>array(
        'query'=>array(
            'name'  => 'announcement',
            'order' => 'id desc',
    ...
    'getListByOrderRefundment' => array(
        'query'=>array(
            ...
            'fields'  => 'c.*,u.username',
            'where'   => 'c.if_del = 0 and c.pay_status != 0',
            'page'   => IReq::get('page') ? IReq::get('page') : 1,
        )
    ),
    ...
    'getListByOrderRefundmentRecycle' => array(
        'query'=>array(
            'name'    => 'refundment_doc as c',
            'join'    => 'left join order as o on c.order_id = o.id left join member as m on m.user_id = c.user_id',
            'fields'  => 'o.order_no,c.amount,m.true_name,c.id,o.id as oid,c.pay_status,c.time',
            'where'   => 'c.if_del = 1 and c.pay_status != 0',
            'order'   => 'c.id desc',
            'page'    => IReq::get('page') ? IReq::get('page') : 1,
        )
    ),
    ...
    'getListRule'=>array(
        'query'=>array(
            'name'  => 'promotion',
            'where' => "type in (0,5) and seller_id = ".(isset(IWeb::$app->getController()->seller['seller_id']) ? IWeb::$app->getController()->seller['seller_id'] : 0),
            'page'  => IReq::get('page') ? IReq::get('page') : 1,
        )
    ),
    ...
    'getRefundmentList'=> array(
        'query'=>array(
            'name'  => 'refundment_doc',
            'where' => 'if_del = 0 and pay_status = 0',
            'order' => 'id desc',
            'page'  => IReq::get('page') ? IReq::get('page') : 1,
        )
    ),
    ...
    //获取所有快递模版
    'getExpresswaybillAll'=> array(
        'query' => array(
            'name'  => 'expresswaybill',
            'fields'=> 'id,name,is_close'
        )
    ),

    //根据商品ID获取品牌数据
    'getBandByGoodsId'=> array(
        'query' => array(
            'name'  => 'brand as b',
    ...
    //根据订单ID获取商品数据
    'getGoodsByOrderId'=> array(
        'query' => array(
            'name'  => 'order_goods as og',
            'join'  => 'left join goods as go on go.id = og.goods_id',
            'where' => 'og.order_id = #order_id#',
            'fields'=>'og.*,go.seller_id'
        )
    ),

    //根据供应商订单ID获取商品数据
    'getSellerGoodsByOrderId'=> array(
        'query' => array(
            'name'  => 'order_goods',
            'where' => 'order_id = #order_id# and seller_id = '.IWeb::$app->getController()->seller['seller_id'],
        )
    ),
    ...
    'goodsCount' => array(
    	'query' => array(
	    	'name' => 'goods',
	    	'fields'=> 'count(*) as num',
	    	'where' => 'is_del in (0,2,3) '.(IWeb::$app->getController()->seller['seller_id'] ? ' and seller_id = '.IWeb::$app->getController()->seller['seller_id'] : ''),
	    	'type'  => 'num',
    	)
    ),
    ...
    'refundsCount' => array(
    	'query' => array(
	    	'name' => 'refundment_doc',
	    	'fields'=> 'count(*) as num',
	    	'where' => 'pay_status = 0 and if_del = 0 '.(IWeb::$app->getController()->seller['seller_id'] ? ' and seller_id = '.IWeb::$app->getController()->seller['seller_id'] : ''),
	    	'type'  => 'num',
    	)
    ),
    ...
    'commentWaitCount' => array(
    	'query' => array(
	    	'name' => 'comment',
	    	'fields'=> 'count(*) as num',
	    	'where' => 'status = 1 and recomment_time = 0 '.(IWeb::$app->getController()->seller['seller_id'] ? ' and seller_id = '.IWeb::$app->getController()->seller['seller_id'] : ''),
	    	'type'  => 'num',
    	)
    ),
    ...
    'referWaitCount' => array(
    	'query' => array(
	    	'name' => 'refer',
	    	'fields'=> 'count(*) as num',
	    	'where' => 'status = 0 '.(IWeb::$app->getController()->seller['seller_id'] ? ' and seller_id = '.IWeb::$app->getController()->seller['seller_id'] : ''),
	    	'type'  => 'num',
    	)
    ),
    ...
    'commentCount' => array(
    	'query' => array(
	    	'name' => 'comment',
	    	'fields'=> 'count(*) as num',
	    	'where' => 'status = 1 '.(IWeb::$app->getController()->seller['seller_id'] ? ' and seller_id = '.IWeb::$app->getController()->seller['seller_id'] : ''),
	    	'type'  => 'num',
    	)
    ),
);