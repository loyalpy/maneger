...
class IClient
{
	...
	{
		if(stripos($_SERVER['HTTP_USER_AGENT'], 'MicroMessenger') && ISession::get('_from') != 'miniProgram' && stripos($_SERVER['HTTP_USER_AGENT'], 'miniProgram') === false)
		{
			return true;
		}
		return false;
	...
	{
		if(stripos($_SERVER['HTTP_USER_AGENT'], 'miniProgram') || ISession::get('_from') == 'miniProgram')
		{
			return true;
		}
		return false;
	...
	/**
	 * @brief 获取当前地理位置
	 * @return array(country=>国家, province=>省份, city=>城市)
	 */
	public static function getLocal()
	{
		$result = file_get_contents('https://int.dpool.sina.com.cn/iplookup/iplookup.php?format=json');
